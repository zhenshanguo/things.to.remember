...
public class Solution {
    public boolean increasingTriplet(int[] nums) {
        ...
        for (int x : nums) {
            if (x <= x1) x1 = x;
            else if (x <= x2) x2 = x; // when x2 is set, there must have been 1 element in front of it and also smaller than it
            else return true; // when x is bigger than x2, we should return true
        }
        return false;
    }
}